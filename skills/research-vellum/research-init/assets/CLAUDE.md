...
- `docs/REQUIREMENTS-{name}.md` — the researcher's intent, in their words, plus
  agreed clarifications. Update it when intent changes.
- `docs/PSEUDOCODE-{name}.md` — structured pseudocode specs (one per experiment
  script/phase). Each starts with YAML frontmatter (title, entry_scripts, dates,
  status). The top-level abstraction the researcher reviews and the primary audit
  baseline. §1a lists searchable hyperparameters, §1b lists fixed experiment
  switches — keep them separate. Keep code in sync with it; when experiment logic
  changes, update it.
- `docs/DIFF-{name}.md` — discrepancy reports generated by Mode B1 (when
  researcher docs exist). Compares documented intent vs actual code behavior.
